...
const FILTER_OPTIONS = ["All", ...INTEREST_LEVELS] as const;
    ...
    >
      ...
      <div className="px-2 pt-2">
        <Select value={interestFilter} onValueChange={setInterestFilter}>
          ...
          >
            <Filter className="w-3 h-3 mr-1 shrink-0 text-muted-foreground" />
          ...
          <SelectContent>
            {FILTER_OPTIONS.map((option) => (
              <SelectItem
                key={option}
                value={option}
                data-testid={`filter-option-${option.toLowerCase()}-${statusSlug}`}
              >
